...
{
	void VehicleEditorCategory::prepare_vehicle()
	{
		if (current_veh != -1) // when == -1 dont change selected slot
			selected_slot = -1;
	...
	void VehicleEditorCategory::Draw()
	{
		if (!Self::GetVehicle())
		{
		...
		ImGui::BeginGroup();
		{
			...
			ImGui::SeparatorText("Mod Options");
			{
				if (ImGui::Checkbox("Burstible tires", (bool*)&owned_mods[(int)CustomVehicleModType::MOD_TIRE_CAN_BURST]))
					FiberPool::Push([this] {
						VEHICLE::SET_VEHICLE_TYRES_CAN_BURST(current_veh, owned_mods[(int)CustomVehicleModType::MOD_TIRE_CAN_BURST]);
					});
				ImGui::SameLine();
				if (ImGui::Checkbox("Low Grip Tires", (bool*)&owned_mods[(int)CustomVehicleModType::MOD_DRIFT_TIRE]))

					FiberPool::Push([this] {
						VEHICLE::SET_DRIFT_TYRES(current_veh, owned_mods[(int)CustomVehicleModType::MOD_DRIFT_TIRE]);
					});
				ImGui::SameLine();
				if (ImGui::Checkbox("Turbo", (bool*)&owned_mods[(int)VehicleModType::MOD_TURBO]))

					FiberPool::Push([this] {
						VEHICLE::TOGGLE_VEHICLE_MOD(current_veh, (int)VehicleModType::MOD_TURBO, owned_mods[(int)VehicleModType::MOD_TURBO]);
					});
				ImGui::SameLine();
				if (ImGui::Checkbox("Tire Smoke", (bool*)&owned_mods[(int)VehicleModType::MOD_TYRE_SMOKE]))

					FiberPool::Push([this] {
						VEHICLE::TOGGLE_VEHICLE_MOD(current_veh, (int)VehicleModType::MOD_TYRE_SMOKE, owned_mods[(int)VehicleModType::MOD_TYRE_SMOKE]);
					});
			}
					...
					ImGui::BeginGroup();
					{
						...
						if (ImGui::BeginListBox("##mod", ImVec2(240, 200)))
						{
							for (const auto& it : mod_display_names[selected_slot])
							{
								...
								{
									FiberPool::Push([&mod, is_wheel_mod, wheel_stock_mod, wheel_custom, name, this] {
										if (selected_slot >= 0)
										{
										...
										else if (selected_slot == (int)CustomVehicleModType::MOD_WHEEL_TYPE)
										{
											...
											VEHICLE::SET_VEHICLE_MOD(current_veh, (int)VehicleModType::MOD_FRONTWHEEL, 0, 0);
											VEHICLE::SET_VEHICLE_MOD(current_veh, (int)VehicleModType::MOD_REARWHEEL, 0, 0);
											current_veh = -1;
										}
							...
							if (ImGui::BeginListBox("##style", ImVec2(200, 200)))
							{
								...
								for (int i = 0; i < wheel_mods.size(); i++)
								{
									...
									if (ImGui::Selectable(("Style " + std::to_string(mod)).c_str(), mod == owned_mods[selected_slot] && *wheel_custom == should_custom))
										FiberPool::Push([&mod, should_custom, this] {
				...
				for (int extra = (int)CustomVehicleModType::MOD_EXTRA_1; extra >= (int)CustomVehicleModType::MOD_EXTRA_14; extra--)
					if (owned_mods.find(extra) != owned_mods.end())
					{
						...
						if (ImGui::Checkbox(std::format("{}###extra{}", id, id).c_str(), &is_extra_enabled))
						{
							...
							FiberPool::Push([id, is_extra_enabled, this] {
			...
			ImGui::SeparatorText("Neon Light Options");
			{
				...
				if (ImGui::Checkbox("Headlight", (bool*)&owned_mods[(int)VehicleModType::MOD_XENON_LIGHTS]))
					FiberPool::Push([this] {
						VEHICLE::TOGGLE_VEHICLE_MOD(current_veh, (int)VehicleModType::MOD_XENON_LIGHTS, owned_mods[(int)VehicleModType::MOD_XENON_LIGHTS]);
					});
				ImGui::PopID();
				ImGui::SameLine();
				if (ImGui::Checkbox("Left", (bool*)&owned_mods[(int)CustomVehicleModType::MOD_NEON_LEFT_ON]))
					FiberPool::Push([this] {
						VEHICLE::SET_VEHICLE_NEON_ENABLED(current_veh, (int)NeonLightLocations::NEON_LEFT, owned_mods[(int)CustomVehicleModType::MOD_NEON_LEFT_ON]);
					});
				ImGui::SameLine();
				if (ImGui::Checkbox("Right", (bool*)&owned_mods[(int)CustomVehicleModType::MOD_NEON_RIGHT_ON]))
					FiberPool::Push([this] {
						VEHICLE::SET_VEHICLE_NEON_ENABLED(current_veh, (int)NeonLightLocations::NEON_RIGHT, owned_mods[(int)CustomVehicleModType::MOD_NEON_RIGHT_ON]);
					});
				ImGui::SameLine();
				if (ImGui::Checkbox("Front", (bool*)&owned_mods[(int)CustomVehicleModType::MOD_NEON_FRONT_ON]))
					FiberPool::Push([this] {
						VEHICLE::SET_VEHICLE_NEON_ENABLED(current_veh, (int)NeonLightLocations::NEON_FRONT, owned_mods[(int)CustomVehicleModType::MOD_NEON_FRONT_ON]);
					});
				ImGui::SameLine();
				if (ImGui::Checkbox("Back", (bool*)&owned_mods[(int)CustomVehicleModType::MOD_NEON_BACK_ON]))
					FiberPool::Push([this] {
						VEHICLE::SET_VEHICLE_NEON_ENABLED(current_veh, (int)NeonLightLocations::NEON_BACK, owned_mods[(int)CustomVehicleModType::MOD_NEON_BACK_ON]);
					});
			}
					...
					if (ImGui::BeginListBox("##colors", ImVec2(140, 254)))
					{
						...
						if (ImGui::Selectable("Remove Custom", false))
							FiberPool::Push([this] {
								...
								VEHICLE::SET_VEHICLE_COLOURS(current_veh, owned_mods[(int)CustomVehicleModType::MOD_PRIMARY_COL], owned_mods[(int)CustomVehicleModType::MOD_SECONDARY_COL]);
							});
				...
				if (color_type == 9)
				{
					...
					if (ImGui::ColorPicker3("Custom Vehicle Color", color, ImGuiColorEditFlags_NoAlpha | ImGuiColorEditFlags_NoDragDrop | ImGuiColorEditFlags_NoOptions | ImGuiColorEditFlags_InputRGB | ImGuiColorEditFlags_DisplayRGB | ImGuiColorEditFlags_DisplayHex))
					{
						...
						*color_b = (int)(color[2] * 255);
						...
						FiberPool::Push([color_r, color_g, color_b, this] {
							...
							case 5:
								...
								break;
							case 8: VEHICLE::SET_VEHICLE_NEON_COLOUR(current_veh, *color_r, *color_g, *color_b); break;
							}
						});
					}
				}
						...
						case 7: //Pearlescent
						{
							for (const auto& [color, name] : lscClassicColors)
							{
								if (ImGui::Selectable(name, selected_color == color))
								{
									...
									FiberPool::Push([this] {
										VEHICLE::SET_VEHICLE_EXTRA_COLOURS(current_veh, owned_mods[(int)CustomVehicleModType::MOD_PEARLESCENT_COL], owned_mods[(int)CustomVehicleModType::MOD_WHEEL_COL]);
									});
								}
							}
							break;
						...
						case 10: //Interior Color
						{
							for (const auto& [color, name] : lscClassicColors)
							{
								if (ImGui::Selectable(name, selected_color == color))
								{
									...
									FiberPool::Push([this] {
										VEHICLE::SET_VEHICLE_EXTRA_COLOUR_5(current_veh, owned_mods[(int)CustomVehicleModType::MOD_INTERIOR_COL]);
									});
								}
							}
							break;
						...
						case 11: //Dashboard Color
						{
							for (const auto& [color, name] : lscClassicColors)
							{
								if (ImGui::Selectable(name, selected_color == color))
								{
									...
									FiberPool::Push([this] {
										VEHICLE::SET_VEHICLE_EXTRA_COLOUR_6(current_veh, owned_mods[(int)CustomVehicleModType::MOD_DASHBOARD_COL]);
									});
								}
							}
							break;
						...
						case 12: //Headlight Color
						{
							for (const auto& [color, name] : lscHeadlightColors)
							{
								if (ImGui::Selectable(name, selected_color == color))
								{
									...
									FiberPool::Push([this] {
										VEHICLE::SET_VEHICLE_XENON_LIGHT_COLOR_INDEX(current_veh, owned_mods[(int)CustomVehicleModType::MOD_XENON_COL]);
									});
								}
							}
							break;
