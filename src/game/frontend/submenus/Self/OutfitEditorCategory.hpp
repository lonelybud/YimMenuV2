...
namespace YimMenu::Submenus
{
	class OutfitEditorCategory : public SubmenuMenuCategory
	{
		...
		void RefreshStats()
		{
			...
			{
				auto& item = t.second;
				item.drawable_id = PED::GET_PED_DRAWABLE_VARIATION(Self::GetPed().GetHandle(), item.id);
				item.drawable_id_max = PED::GET_NUMBER_OF_PED_DRAWABLE_VARIATIONS(Self::GetPed().GetHandle(), item.id) - 1;
				item.texture_id = PED::GET_PED_TEXTURE_VARIATION(Self::GetPed().GetHandle(), item.id);
				item.texture_id_max = PED::GET_NUMBER_OF_PED_TEXTURE_VARIATIONS(Self::GetPed().GetHandle(), item.id, item.drawable_id) - 1;
			}

			for (auto& t : props.items)
			{
				auto& item = t.second;
				item.drawable_id = PED::GET_PED_PROP_INDEX(Self::GetPed().GetHandle(), item.id, 0);
				item.drawable_id_max = PED::GET_NUMBER_OF_PED_PROP_DRAWABLE_VARIATIONS(Self::GetPed().GetHandle(), item.id) - 1;
				item.texture_id = PED::GET_PED_PROP_TEXTURE_INDEX(Self::GetPed().GetHandle(), item.id);
				item.texture_id_max = PED::GET_NUMBER_OF_PED_PROP_TEXTURE_VARIATIONS(Self::GetPed().GetHandle(), item.id, item.drawable_id) - 1;
			}
		}
